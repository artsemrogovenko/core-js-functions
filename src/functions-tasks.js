...
function logger(func, logFunc) {
  return (...args) => {
    const str = args.map((a) => JSON.stringify(a));
...
function partialUsingArguments(fn, ...args1) {
  return (...args) => {
    const flat = [args1, args].flat();
    const result = fn.apply(this, flat);
    return result;
  };
}
